    ...
    <div className="space-y-8 bg-white p-6 rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 border-b pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Özellik Adı</label>
            <input
              name="name"
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-800">Seçenekler / Değerler</h2>
        </div>
        ...
        <ul className="divide-y divide-gray-200 border rounded-md">
          {attribute.values && attribute.values.map((val) => (
            <li key={val.id} className="flex justify-between items-center p-3 hover:bg-gray-50">
              <span>{val.name}</span>
              <button
                type="button"
          ...
          {(!attribute.values || attribute.values.length === 0) && (
            <li className="p-4 text-center text-gray-500">Henüz değer eklenmemiş.</li>
          )}
        </ul>
      </div>
    </div>
