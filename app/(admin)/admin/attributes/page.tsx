        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Değerler</th>
            ...
            {attributes.length > 0 ? (
              attributes.map((attr: any) => (
                <tr key={attr.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{attr.slug}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">
                    <div className="flex flex-wrap gap-1">
                      {attr.values?.map((val: any) => (
                        <span key={val.id} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                          {val.name}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                    <ToggleAttributeActiveButton attributeId={attr.id} isActive={true} />
                    <Link href={`/admin/attributes/${attr.id}/edit`} className="text-indigo-600 hover:text-indigo-900">Düzenle</Link>
              ...
              <tr>
                <td colSpan={4} className="px-6 py-4 text-center text-gray-500">Kayıtlı özellik bulunamadı.</td>
              </tr>
            )}
          </tbody>
        </table>
