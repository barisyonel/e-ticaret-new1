...
export default function CouponList({ initialCoupons }: CouponListProps) {
  ...
  const handleToggleActive = async (coupon: Coupon) => {
    ...
    if (result.success) {
      ...
      showToast(
        result.error || "Kupon güncellenirken bir hata oluştu",
        "error",
      );
    }
    setTogglingId(null);
                ...
                <td className="px-6 py-4">
                  <div className="text-sm">
                    <p className="text-gray-900">
                      Başlangıç: {formatDateToTurkey(coupon.validFrom)}
                    </p>
                    {coupon.validUntil && (
                      <p
                        className={`text-gray-600 ${isExpired(coupon) ? "text-red-600 font-semibold" : ""}`}
                      >
