...
export default async function AdminReviewsPage() {
  ...
  const reviewsData =
    reviewsResult.success && reviewsResult.data
      ? reviewsResult.data
      : { reviews: [], total: 0, page: 1, limit: 50 };

  // Calculate statistics
  const totalReviews = reviewsData.total;
  const approvedReviews = reviewsData.reviews.filter(
    (r) => r.isApproved,
  ).length;
  const pendingReviews = reviewsData.reviews.filter(
    (r) => !r.isApproved,
  ).length;
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Ürün yorumlarını görüntüleyin ve yönetin
        </p>
      </div>
