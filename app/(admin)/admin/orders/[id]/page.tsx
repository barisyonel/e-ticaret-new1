...
} from "@/app/server-actions/orderActions";
...
export default async function AdminOrderDetailPage({ params }: PageProps) {
  ...
  const shippingAddress = OrderRepository.parseShippingAddress(
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {order.items.map((item: any) => {
                      const productImage =
                        item.productImage || "/placeholder-image.svg";
                      return (
                        <tr key={item.id}>
