...
BEGIN
    ...
    BEGIN
        ...
        WHILE @@FETCH_STATUS = 0
        BEGIN
            -- Her alt kategori için unique slug oluştur (alt kategori slug'ının son 20 karakterini kullan)
            DECLARE @SubSlugShort NVARCHAR(20) = RIGHT(@SubSlug, 20);
            DECLARE @UniqueChildSlug NVARCHAR(255) = CONCAT(@ChildSlug, '-', @SubSlugShort);
            
            -- Slug çok uzun olursa kısalt
            IF LEN(@UniqueChildSlug) > 200
            BEGIN
                SET @UniqueChildSlug = CONCAT(@ChildSlug, '-', SUBSTRING(@SubSlug, 1, 30));
            END
